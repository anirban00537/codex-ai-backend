...
import { GoogleSignInDto, GoogleSignInWithAccessTokenDto } from './dto/googleCred.dto';
...
export class AuthController {
  ...
  @Public()
  @Post('google-login-with-access-token')
  async googleLoginWithAccessToken(
    @Req() request: Request,
    @Body() googleCred: GoogleSignInWithAccessTokenDto,
  ) {
    const browserInfo =
      `${request.ip} ${request.headers['user-agent']} ${request.headers['accept-language']}`.replace(
        / undefined/g,
    ...
    return this.authService.googleLoginWithAccessToken(
      request,
      googleCred,
      browserInfo,
    );
